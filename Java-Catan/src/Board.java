...
public class Board {
    ...
    public void makeDefaltBoard(int Dimension1,int Dimension,int Dimension3){
        start=new Hex("Start");
        int numberOfHexs=1;
       ...
       // start.combineHex(wire,1);
        ...
        building.add(start);
        for (int i = building.size(); i < Dimension*building.size(); i++) {
    ...
    //    System.out.println(start.nearbyHex[5].toString());
        ...
        for (int i = building.size(); i < Dimension3*building.size(); i++) {
            Hex pop=building.poll();
            Hex temp=new Hex();
